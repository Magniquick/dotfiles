...
Item {
  ...
  function parseServers(text) {
    if (!text)
      return []
    const parsed = JsonUtils.parseArray(text)
    if (parsed)
      return parsed
    console.warn("Failed to parse MCP config")
    return []
  }
  ...
  function addServer(url, label) {
    ...
    if (!parsed)
      return {
        ok: false,
        error: qsTr("Enter a valid http:// or https:// MCP endpoint.")
      }
    ...
    for (let i = 0; i < current.length; i++) {
      ...
      if (existing === parsed.value)
        return {
          ok: false,
          error: qsTr("That MCP server URL is already configured.")
        }
    }
    ...
    if (!saved)
      return {
        ok: false,
        error: qsTr("Failed to save MCP server configuration.")
      }
