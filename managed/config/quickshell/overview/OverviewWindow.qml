...
Item { // Window
    ...
    property string iconName: {
        ...
        // For PWA-style classes (e.g. "brave-web.whatsapp.com__-Default"), try the browser icon
        const cls = `${windowData?.class ?? ""}`;
        const browserMatch = cls.match(/^(brave|chromium|chrome|firefox|edge)[-_]/i);
        if (browserMatch) {
            const map = { "brave": "brave-desktop", "chromium": "chromium-browser", "chrome": "google-chrome", "firefox": "firefox", "edge": "microsoft-edge" };
            return map[browserMatch[1].toLowerCase()] || browserMatch[1].toLowerCase();
        }

        return "application-x-executable";
    }
    property var iconPath: Quickshell.iconPath(iconName, "application-x-executable")
