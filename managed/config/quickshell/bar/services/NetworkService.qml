...
Item {
  ...
  function updateIpAddressDetails(text) {
    ...
    const entries = JsonUtils.parseArray(text) || []
  ...
  CommandRunner {
    ...
    enabled: root.connectionType === "ethernet" && root.connectionState === "connected" && root.deviceName !== "" && root.subsystemCheckRequested
    intervalMs: 0
  ...
  CommandRunner {
    ...
    enabled: root.connectionType === "ethernet" && root.connectionState === "connected" && root.deviceName !== "" && root.ethernetSubsystem === "usb" && root.deviceLabelRequested
    intervalMs: 0
