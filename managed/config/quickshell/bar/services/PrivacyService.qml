  ...
  Timer {
    ...
    onTriggered: {
      ...
      for (let i = 0; i < Pipewire.nodes.values.length; i++) {
        ...
        console.log("[PrivacyService] node", mediaClass, "name=" + (node.name || ""), "app=" + (node.properties["application.name"] || ""), "media.name=" + (node.properties["media.name"] || ""), "live=" + (node.properties["stream.is-live"] || ""), "state=" + root.stateString(node.state))
