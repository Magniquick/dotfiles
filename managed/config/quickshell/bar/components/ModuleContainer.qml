...
Rectangle {
  ...
  readonly property color surfaceColor: root.backgroundTransparent
    ? "transparent"
    : (root.hovered ? Config.moduleBackgroundHover : root.backgroundColor)
  ...
  readonly property int contentImplicitHeight: Math.max(
    contentRow.implicitHeight + root.paddingTop + root.paddingBottom,
    root.minHeight
  )
  ...
  Rectangle {
    ...
    color: "transparent"
    radius: Math.min(width, height) / 2
    border.width: (root.backgroundTransparent || !root.hovered) ? 0 : Config.tooltipBorderWidth
