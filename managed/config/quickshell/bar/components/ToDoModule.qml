...
ColumnLayout {
  ...
  property var projects: ["Inbox", "Today"]
  ...
  onCurrentProjectChanged: root.updateTasks()
  ...
  function applyTodoistData(data, fromCache) {
    ...
    if (idx === -1) {
      const todayIndex = projects.indexOf("Today")
      root.currentProject = projects[todayIndex !== -1 ? todayIndex : 0] || "Today"
    }
  ...
  function taskCountLabel(count) {
    return count === 1 ? qsTr("1 Task") : qsTr("%1 Tasks").arg(count)
  }
  ...
  RowLayout {
    ...
    ColumnLayout {
      ...
      Text {
        ...
        text: root.loading ? qsTr("Loading tasks...") : (root.parseError ? (root.tasks.length > 0 ? qsTr("%1 (last sync)").arg(root.taskCountLabel(root.tasks.length)) : qsTr("Tasks unavailable")) : root.taskCountLabel(root.tasks.length))
      }
      Text {
        color: Config.color.on_surface_variant
        font.family: Config.fontFamily
        font.pixelSize: Config.type.labelMedium.size
        text: root.loading ? qsTr("Fetching from Todoist...") : (root.parseError ? (root.tasks.length > 0 ? qsTr("Todoist error - showing last sync.") : qsTr("Todoist error - no data.")) : qsTr("remaining to be completed."))
      }
    }
    Item {
      Layout.fillWidth: true
  ...
  RowLayout {
    ...
    ComboBox {
      ...
      currentIndex: Math.max(0, root.projects.indexOf(root.currentProject))
      ...
      onActivated: index => {
        root.currentProject = root.projects[index] || "Today"
        root.updateTasks()
      }
      popup: Popup {
