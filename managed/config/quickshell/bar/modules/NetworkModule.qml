...
ModuleContainer {
    ...
    function applyEthernetSubsystem(subsystem) {
        root.ethernetSubsystem = subsystem || "";
        root.subsystemCheckRequested = false;
        if (root.connectionType !== "ethernet" || root.connectionState !== "connected")
            return;
        root.iconText = root.ethernetSubsystem === "usb" ? root.usbEthernetIcon : root.ethernetIcon;
        if (root.ethernetSubsystem === "usb") {
    ...
    CommandRunner {
        ...
        onOutputChanged: {
            root.ethernetDeviceLabel = output.replace(/_/g, " ");
            root.deviceLabelRequested = false;
        }
    }
