...
Rectangle {
  ...
  color: root.active
    ? Config.moduleBackgroundMuted
    : (root.hovered ? Config.moduleBackgroundHover : "transparent")
  radius: height / 2
  antialiasing: true
  implicitHeight: Config.workspaceHeight
  implicitWidth: (root.uniformWidth
    ? Math.max(labelText.implicitWidth, root.uniformTextWidth)
    : labelText.implicitWidth) + root.paddingX * 2
  ...
  Text {
    ...
    color: root.urgent
      ? Config.warn
      : (root.active ? Config.accent : Config.textMuted)
