...
ModuleContainer {
  ...
  function updateLyricsModel() {
    ...
    if (!selected.lines || selected.lines.length === 0) {
      ...
      if (!loading && spotifyRef === "" && root._lyricsLookupKey === "")
        ...
        root.lyricsModel = [
          {
            text: loading ? "Loading lyrics..." : "♪ No lyrics available ♪",
            isCurrent: true
          }
        ]
      root.currentLyricIndex = -1
        ...
        Item {
          ...
          ListView {
            ...
            delegate: Item {
              ...
              implicitHeight: hasWordSegments ? Math.max(fallbackText.implicitHeight, segmentRowsColumn.implicitHeight) : fallbackText.implicitHeight
            ...
            Connections {
              ...
              function onCurrentLyricIndexChanged() {
                lyricsView.followCurrent()
              }
              function onTooltipActiveChanged() {
                if (root.tooltipActive)
                  lyricsView.followCurrent()
              }
            }
            ...
            Connections {
              target: root
              function onActiveLyricsLinesChanged() {
                lyricsView.followCurrent()
              }
            }
          }
  ...
  Connections {
    ...
    function onLoadedChanged() {
      ...
      if (lyricsClient.loaded) {
        console.log("[MprisModule] lyrics loaded", "lookupKey=" + resolvedKey, "source=" + String(lyricsClient.source || ""), "provider=" + String((lyricsClient.metadata || {}).provider || ""), "syncType=" + String(lyricsClient.syncType || ""), "lines=" + String((lyricsClient.lines || []).length))
  ...
  Connections {
    ...
    function onPositionChanged() {
      root._syncPositionBaseWithPendingGuard()
    }
