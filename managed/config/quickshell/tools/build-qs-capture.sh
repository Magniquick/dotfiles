...
MODULE_DIR="$ROOT_DIR/common/modules/qs-capture"
BUILD_DIR="$MODULE_DIR/build"

if [[ ! -d "$MODULE_DIR" ]]; then
  echo "qs-capture module not found: $MODULE_DIR" >&2
  exit 1
fi

if ! command -v ninja >/dev/null 2>&1; then
  echo "ninja not found in PATH; install ninja to build qs-capture" >&2
  exit 1
fi
