...
RowLayout {
  ...
  readonly property string sourceText: (root.showSourceButton && root.showSourceDetails) ? buildSourceText(entry) : ""
  ...
  readonly property bool bodyOverflows: root.bodyMaxLines > 0 && root.cleanBody.length > 0 && bodyMeasure.lineCount > root.bodyMaxLines
  ...
  readonly property bool hasDefaultAction: (root.entry?.notification?.actions ?? []).some(action => (action && action.identifier ? String(action.identifier) : "") === "default")
  ...
  function actionDisplayText(action) {
    ...
    return text.length > 0 ? text : qsTr("Activate")
  }
  ...
  ColumnLayout {
    ...
    RowLayout {
      ...
      ColumnLayout {
        ...
        CommonComponents.LinkText {
          ...
          maximumLineCount: (root.bodyMaxLines > 0 && root.bodyExpandable) ? ((root._hoverExpandActive || root._manualExpandActive) ? root._expandedBodyLines : root._collapsedBodyLines) : 0
    ...
    RowLayout {
      ...
      Repeater {
        id: actionsRepeater
        model: (root.entry?.notification?.actions ?? []).filter(action => (action && action.identifier ? String(action.identifier) : "") !== "default")
    ...
    RowLayout {
      ...
      MK.ClickableSurface {
        ...
        Text {
          anchors.centerIn: parent
          text: "\uf1d8"
          color: replyButton.hovered ? Common.Config.color.primary : Common.Config.color.on_surface_variant
          font.family: Common.Config.iconFontFamily
          font.pointSize: 10
        }
      }
