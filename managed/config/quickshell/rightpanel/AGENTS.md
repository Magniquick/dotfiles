...
Root `shell.qml` -> `RightPanel.qml` -> notification components

- **Root `shell.qml`**: Creates the right `PanelWindow`, owns `HyprlandFocusGrab`, and closes via `GlobalState.rightPanelVisible`.
- **RightPanel.qml**: Notification center content with `NotificationServer` integration and a secondary popup overlay `PanelWindow`.
