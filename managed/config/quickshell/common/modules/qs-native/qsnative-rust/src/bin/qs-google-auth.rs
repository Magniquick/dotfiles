...
fn provision(args: &[String]) -> Result<(), String> {
    let opts = parse_flags(args, true)?;
    let account = app_config::load_account(&opts.config_path, &opts.account_id)?;
    google_auth::provision(&account, &opts.client_json)?;
    eprintln!(
...
fn list_calendars(args: &[String]) -> Result<(), String> {
    let opts = parse_flags(args, false)?;
    let account = app_config::load_account(&opts.config_path, &opts.account_id)?;
...
fn parse_flags(args: &[String], needs_client: bool) -> Result<Options, String> {
    ...
    if opts.account_id.trim().is_empty() {
