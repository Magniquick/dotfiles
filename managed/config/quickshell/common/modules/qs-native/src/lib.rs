...
fn sync_aur_database(network_interval_seconds: u32) -> Result<(), String> {
    let mut cache = aur_cache().lock().map_err(|_| "AUR cache lock failed".to_string())?;
    ...
    let package_names: Vec<&str> = local_packages.iter().map(|(name, _)| name.as_str()).collect();
...
fn get_aur_updates() -> Result<(u16, String), String> {
    let cache = aur_cache().lock().map_err(|_| "AUR cache lock failed".to_string())?;
