...
ModuleContainer {
    ...
    ColumnLayout {
      ...
      ColumnLayout {
        ...
        Text {
          ...
          text: root.inhibitEnabled ? root.activeSubtitle() : root.dpmsTimeoutSeconds > 0 ? "Display off after " + root.timeoutLabel(root.dpmsTimeoutSeconds) + " idle" : "Display idle timeout disabled"
      ...
      ColumnLayout {
        ...
        InfoRow {
          Layout.fillWidth: true
          label: "Lid close"
          value: GlobalState.idleIgnoreLidEvents ? "Ignored" : "Normal"
          valueColor: GlobalState.idleIgnoreLidEvents ? Config.color.primary : Config.color.on_surface
        }
      }
      ...
      TooltipActionsRow {
        ...
        ActionChip {
          Layout.fillWidth: true
          active: GlobalState.idleIgnoreLidEvents
          text: "Ignore Lid"
          onClicked: GlobalState.toggleIdleIgnoreLidEvents()
        }
      }
    }
  }
  ...
  function headerSubtitle() {
    if (root.inhibitEnabled && GlobalState.idleIgnoreLidEvents)
      return "Awake mode + lid ignored"
    if (root.inhibitEnabled)
      return "Awake mode enabled"
    if (GlobalState.idleIgnoreLidEvents)
      return "Lid close ignored"
    return "Auto sleep enabled"
  }
