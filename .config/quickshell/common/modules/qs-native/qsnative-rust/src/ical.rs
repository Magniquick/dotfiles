...
async fn fetch_all_sources(
    ...
    for source in &sources {
        let mut hub = match google_auth::calendar_hub_silent(&source.account_id).await {
            Ok(hub) => hub,
            Err(error) => {
        ...
        for calendar_id in &source.calendar_ids {
            match fetch_calendar_events(&hub, calendar_id, range_start, range_end).await {
                ...
                Err(error) if is_calendar_auth_error(&error) && !reauthorized => {
                    eprintln!(
                        "[IcalCache] {}: calendar API returned 401; reauthorizing",
                        source.account_id
                    );
                    reauthorized = true;
                    hub = match google_auth::reauthorize_calendar_hub(&source.account_id).await {
                        Ok(hub) => hub,
                        Err(reauth_error) => {
                            errors.push(format!(
                                "{}/{}: reauthorize: {reauth_error}",
                                source.account_id, calendar_id
                            ));
                            continue;
                        }
                    };
...
async fn fetch_calendar_events<C>(
    ...
    range_start: DateTime<Local>,
    range_end: DateTime<Local>,
) -> Result<Vec<ParsedEvent>, calendar3::common::Error>
where
    C: calendar3::common::Connector,
{
    ...
    loop {
        ...
        let response = call.doit().await.map(|(_, value)| value)?;
...
fn is_calendar_auth_error(error: &calendar3::common::Error) -> bool {
    match error {
        calendar3::common::Error::BadRequest(value) => google_error_code(value) == Some(401),
        calendar3::common::Error::Failure(response) => {
            response.status() == calendar3::hyper::StatusCode::UNAUTHORIZED
        }
        _ => false,
    }
}

fn google_error_code(value: &serde_json::Value) -> Option<u64> {
    value
        .get("error")
        .and_then(|error| error.get("code"))
        .and_then(serde_json::Value::as_u64)
        .or_else(|| value.get("code").and_then(serde_json::Value::as_u64))
}
...
#[cfg(test)]
mod tests {
    use super::{event_from_api, is_calendar_auth_error};
    ...
    #[test]
    fn calendar_auth_error_only_matches_401() {
        assert!(is_calendar_auth_error(
            &google_calendar3::common::Error::BadRequest(json!({
                "error": {
                    "code": 401,
